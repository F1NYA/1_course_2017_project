...
{
    Uint32 *target_pixel = (Uint8 *) surface->pixels + y * surface->pitch + x * sizeof *target_pixel;
    *target_pixel = pixel;
}
...
static void vline(int x, int y1,int y2, int top,int middle,int bottom)
{
    ...
    y2 = clamp(y2, 0, H-1);
    if(y2 == y1)
        //pix[4 * ( y1 * surface->w ) + x] = middle;
        set_pixel(surface, x, y1, middle);
    else if(y2 > y1)
    {
        //pix[y1*W+x] = top;
        set_pixel(surface, x, y1, top);
        for(int y=y1+1; y<y2; ++y) set_pixel(surface, x, y, middle);;
        //pix[y2*W+x] = bottom;
        set_pixel(surface, x, y2, bottom);
    }
